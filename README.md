...
* `docker build -f bee -t bee .`
* `docker run --name bee -p 5001:5001 -v "$(pwd)":/bee -v [MEDIA LIBRARY ROOT PATH]:/library -v /etc/localtime:/etc/locatime -d bee`
...
## Known Issues 🐝
* Because BEE relies on quickly passing files across your local network. WiFi speeds may not be adequate to handle the task and it is recommended the BEE host machine, media shares and Drone devices all be connected to your network over ethernet instead of WiFi.
